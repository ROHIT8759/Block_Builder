...
export function Navbar() {
  ...
  const setWalletAddressStore = useBuilderStore((state) => state.setWalletAddress)
  ...
  const { walletAddress, connect, disconnect, isConnecting, chainId, isFreighterInstalled } = useWallet()
  const lastInitializedAddress = useRef<string | null>(null)

  useEffect(() => {
    if (!walletAddress) {
      setWalletAddressStore(null)
      setWalletChainId(null)
      lastInitializedAddress.current = null
      return
    }

    setWalletAddressStore(walletAddress)
    setWalletChainId(chainId ?? null)

    if (lastInitializedAddress.current === walletAddress) {
      return
    }

    lastInitializedAddress.current = walletAddress
    initializeUser(walletAddress).catch((err) => {
      console.error("Failed to initialize user with wallet", err)
    })
  ...
  const handleConnectWallet = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Unable to connect to Freighter"
      alert(message)
    }
  }
      ...
      <nav className="bg-card border-b border-border px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between animate-fade-in-down">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <button
            ...
            title="Deploy to Stellar mainnet or testnet"
          >
            ...
            <span className="relative hidden sm:inline">Deploy to Stellar</span>
